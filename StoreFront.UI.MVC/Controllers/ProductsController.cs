...
namespace StoreFront.UI.MVC.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("ProductId,ProductName,ProductPrice,ProductDescription,ProductQuantity,ProductOnOrder,CategoryId,ProductImage,Image")] Product product)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    //Check the file type
                    //- retrieve the extension of th uploaded file
                    ...
                    if (validExt.Contains(ext.ToLower()) && product.Image.Length < 4_194_303)//underscores don't change the number, they just make it easier to read.
                    {
                        //Generate a unique filename
                        product.ProductImage = Guid.NewGuid() + ext;

                        //Save the file to the web server (here, saving to wwwroot/images)
                        //To access wwwroot, add a property to the controller for the _weHostEnvironment (see the top of this class for our example)
                        ...
                        using (var memoryStream = new MemoryStream())
                        {
                            ...
                            using (var img = Image.FromStream(memoryStream)) //add a using statement for the Image class (using System.Drawing)
                            {
                                ...
                                //2. (int) max thumbnail image size
